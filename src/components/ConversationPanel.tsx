    ...
    <div className="flex items-start gap-2.5">
      ...
      <div className="max-w-[84%] rounded-2xl rounded-tl-sm px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap bg-gray-50 text-gray-900 border border-gray-200">
        ...
        {message.attribution && message.attribution.length > 0 && (
          <FlowTrace agentName={message.agentName} attribution={message.attribution} />
        )}
      </div>
    </div>
  );
...
const TYPE_TEXT: Record<AgentType, string> = {
...
function FlowPill({ label, cls }: { label: string; cls: string }) {
  return (
...
function FlowTrace({ agentName, attribution }: { agentName?: string; attribution: MessageAttribution[] }) {
  const brokerLabel = agentName ?? 'Broker';
  const single = attribution.length === 1;
  const outputLabel = single
    ? `Generated by ${attribution[0].name}`
    : `Combined from ${attribution.map((a) => a.name).join(' + ')}`;

  return (
    <div className="mt-3 pt-3 border-t border-gray-200 space-y-2.5 not-italic">
      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Flow Trace</p>

      {/* Path */}
      <div className="flex items-center gap-1 flex-wrap">
        <FlowPill label="User" cls="bg-sky-50 text-sky-700 border-sky-200" />
        <FlowArrow />
        <FlowPill label={brokerLabel} cls="bg-blue-50 text-blue-700 border-blue-200" />
        {attribution.map((a, i) => (
          <span key={`path-${i}`} className="flex items-center gap-1">
            <FlowArrow />
            <FlowPill label={a.name} cls={TYPE_BADGE[a.type] ?? 'bg-gray-50 text-gray-600 border-gray-200'} />
          </span>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        <FlowRow
          ...
          detail={`Routed to: ${attribution.map((a) => a.name).join(', ')}`}
        />
        {attribution.map((a, i) => (
          <FlowRow
            key={`row-${i}`}
            label={a.name}
            labelCls={TYPE_TEXT[a.type] ?? 'text-gray-500'}
            detail="Processed request and returned results"
          />
        ))}
        <FlowRow label="Output" labelCls="text-gray-500" detail={outputLabel} />
      </div>
    </div>
  );
}

function FlowRow({ label, labelCls, detail }: { label: string; labelCls: string; detail: string }) {
  return (
    <div className="flex items-start gap-2 text-[11px]">
      <span className={`font-semibold shrink-0 w-28 truncate leading-relaxed ${labelCls}`}>{label}</span>
      <span className="text-gray-500 leading-relaxed">{detail}</span>
    </div>
  );
}
